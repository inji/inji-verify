...
class VerifiablePresentationRequestServiceImplTest {
    ...
    static DcqlQueryScopeRepository mockDcqlQueryScopeRepository;
    ...
    public static void beforeAll() {
        mockDcqlQueryScopeRepository = mock(DcqlQueryScopeRepository.class);
        ...
        service = new VerifiablePresentationRequestServiceImpl(mockDcqlQueryScopeRepository, mockAuthorizationRequestCreateResponseRepository, mockVPSubmissionRepository, mockKeyManagementService);
        service.verifyServiceBaseUrl = "https://verify.example.com";
    }
    ...
    public void shouldCreateNewAuthorizationRequest() throws Exception {
        ...
        VPRequestCreateDto vpRequestCreateDto = new VPRequestCreateDto(
                ...
                objectMapper.readTree("{\"credentials\":[]}"), false, false);
    ...
    void getVPRequestJwt_ValidRequest_ReturnsJwtString() throws JOSEException, VPRequestNotFoundException {
        ...
        AuthorizationRequestResponseDto authzDetailsDto =
                new AuthorizationRequestResponseDto(verifierDid, null, null, null, null, null, false, false);
    ...
    @Test
    void getVPRequestJwt_WithDcqlQueryUri_ReturnsJwt() throws Exception {
        String requestId = "reqWithUri";
        AuthorizationRequestResponseDto authzDto =
                new AuthorizationRequestResponseDto("did:example", "age_verification", "dcqlQueryUri", null, "nonce", "responseUri", false, false);
    ...
    void getVPRequestJwt_WithDcqlQuery_ReturnsJwt() throws Exception {
        ...
        AuthorizationRequestResponseDto authzDto =
                new AuthorizationRequestResponseDto("did:example", null, null, dcqlQuery, "nonce", "responseUri", false, false);
        AuthorizationRequestCreateResponse response = new AuthorizationRequestCreateResponse(requestId, "tx", authzDto, Instant.now().toEpochMilli() + 1000);
        when(mockAuthorizationRequestCreateResponseRepository.findById(requestId)).thenReturn(Optional.of(response));
        OctetKeyPair mockOKP = new OctetKeyPairGenerator(Curve.Ed25519).generate();
        when(mockKeyManagementService.getKeyPair()).thenReturn(mockOKP);
        ...
        AuthorizationRequestCreateResponse response =
                new AuthorizationRequestCreateResponse(requestId, "tx",
                        new AuthorizationRequestResponseDto("did:example",
                                null, null, null, "nonce", "responseUri", false, false), Instant.now().toEpochMilli() + 2000);
        when(mockAuthorizationRequestCreateResponseRepository.findById(requestId)).thenReturn(Optional.of(response));
    ...
    public void shouldCreateNewAuthorizationRequestWithResponseCodeValidationRequired() throws Exception {
        ...
        VPRequestCreateDto vpRequestCreateDto = new VPRequestCreateDto(
                "test_client_id", "test_transaction_id", null, "",
                objectMapper.readTree("{\"credentials\":[]}"), false, true);
    ...
    public void shouldCreateNewAuthorizationRequestWithPresentationFlowCrossDevice() throws Exception {
        ...
        assertNotNull(responseDto);
        assertNull(responseDto.getRequestUri());
    }

    @Test
    public void shouldCreateAuthorizationRequestFromScope() throws DcqlQueryScopeNotFoundException {
        DcqlQueryScope dcqlQueryScope = new DcqlQueryScope("age_verification", "{\"credentials\":[]}", Instant.now());
        when(mockDcqlQueryScopeRepository.findById("age_verification")).thenReturn(Optional.of(dcqlQueryScope));
        when(mockAuthorizationRequestCreateResponseRepository.save(any(AuthorizationRequestCreateResponse.class))).thenReturn(null);

        VPRequestCreateDto vpRequestCreateDto = new VPRequestCreateDto(
                "test_client_id", "test_transaction_id", "age_verification", "", null, false, false);

        VPRequestResponseDto responseDto = service.createAuthorizationRequest(vpRequestCreateDto);

        assertNotNull(responseDto);
        assertEquals("age_verification", responseDto.getAuthorizationDetails().getScope());
        assertNotNull(responseDto.getAuthorizationDetails().getDcqlQueryUri());
    }
}
