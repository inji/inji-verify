...
- Supported VC format: LDP VC (Linked Data Proof Verifiable Credential).

## API Documentation
API documentation is available [here 🔗](https://mosip.stoplight.io/docs/inji-verify/branches/main/)

## Functionalities
### Authorization Request Creation:
- Inji Verify generates an authorization request through the backend, which is rendered as a QR code.
- The QR code is scanned by the wallet to initiate the VP flow.
  ...
  - The Inji Verify UI sends a request to the backend to create the authorization request and renders the response as a QR code.
...
sequenceDiagram
    ...
    Verify UI->>Verify Backend: 1. Create Authorization Request (BACKEND_URL/vp-session-request)
    Verify Backend--)Verify Backend: 2. Process request and generate response
    ...
    Wallet--)Wallet: 7. Process request and select matching VCs
    ...
    Verify UI->>Verify Backend: 11. Fetch result (BACKEND_URL/vp-session-result) [transactionId from HttpOnly Cookie]
