...
D --> E[User Authentication (Biometric / Passcode)]
...
J --> K[User Accepts Consent to Share Selected VC with Verifier]
...
Q --> R[Return redirect_uri with response_code (Same Device Flow)]
R --> S[Verifier UI uses response_code + transactionId (from HttpOnly cookie) to fetch result]
