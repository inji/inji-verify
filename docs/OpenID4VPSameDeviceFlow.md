...
2. **Authorization Request**: The Verifier App constructs an authorization request that contains several parameters like
   ...
   * **responseCodeValidationRequired**- It determines whether the response code validation in redirect_uri is required or not.
   Default value is "false".
   - Cross Device Flow - responseCodeValidationRequired = false
   - Same Device Flow with Mobile Wallet - responseCodeValidationRequired = false
   - Same Device Flow with Web Wallet - responseCodeValidationRequired = true
   - If ```responseCodeValidationRequired``` is true then /vp-submission end point will respond with the redirect_uri containing a response_code other wise it will respond with a plain 200 OK (no body).
...
6. **Transmission of Authorization Response**: Once the Wallet prepares the VP, Wallet sends it back to verifier application (using redirect URI) based on the response_mode and the response_type specified by the verifier application in the Authorization Request
   ...
   > The endpoint can return a **_redirect_uri_** based on the **_INJI_VERIFY_REDIRECT_URI_** configuration and responseCodeValidationRequired = true.
